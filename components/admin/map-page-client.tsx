...
import { STATUS_CONFIG } from "@/lib/project-map-data";
import type { NodeStatus } from "@/lib/project-map-data";
...
export function MapPageClient() {
  ...
  return (
    <div className={`admin-${theme}`} style={{ backgroundColor: "var(--adm-bg)", color: "var(--adm-text)" }}>
      {/* Header */}
      <header
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--adm-text-muted)" }}>
              Level 1 — 6 nodes · Click a node to inspect
            </p>
          </div>
        </div>
      ...
      {/* Map canvas */}
      <div style={{ height: "calc(100dvh - 57px)" }}>
        <ProjectMapView theme={theme} />
      </div>
    </div>
  );
}
