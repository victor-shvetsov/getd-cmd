...
export const MAP_NODES: Node<MapNodeData>[] = [
  ...
  // ── AUTOMATIONS ENGINE ───────────────────────────────────────────────────
  {
    ...
    data: {
      ...
      level2: {
        subNodes: [
          { id: "l2-cron",     type: "subNode", position: { x: 130, y: 0   }, data: { label: "check-leads (cron)", type: "api-route",  hint: "IMAP poll every 5 min — Phase 1: queue · Phase 2: inbox" } },
          ...
          { id: "l2-lead",     type: "subNode", position: { x: 0,   y: 210 }, data: { label: "Lead Reply",         type: "concept",
            details: { description: "When a new email lead arrives, Claude writes a personal reply in the client's voice and either sends it immediately or stores it as a draft. Casper can approve drafts from the portal or by replying OK to an SMS.", targetState: ["IMAP polling captures inbound emails every 5 min", "Returning contacts skipped — only first-time leads get auto-replied", "Reply in Casper's voice using voice samples from corpus", "Draft mode: pending_approval → web or SMS approval", "SMS notification via Twilio — reply OK to approve, SKIP to discard", "All inbound/outbound emails logged to lead_conversations for voice training"], plan: ["Test with real inbound email to Casper's inbox", "Link voice samples from Knowledge Hub into automation config automatically"], keyFiles: ["lib/automations/lead-reply/index.ts", "lib/automations/lead-reply/workflow.ts", "lib/automations/lead-reply/tools.ts", "lib/automations/lead-reply/parse-email.ts", "app/api/cron/check-leads/route.ts", "app/api/webhooks/twilio/route.ts", "app/api/admin/clients/[id]/conversations/"], skill: "/focus lead-reply", changelog: ["WAT architecture scaffolded — index.ts, workflow.ts, tools.ts, parse-email.ts", "IMAP polling replaces Resend inbound webhook — ImapFlow + cron", "Returning contacts check: skip auto-reply, log inbound continuation for corpus", "Delayed queue: reply_delay_minutes → queued → Phase 1 processes on next tick", "Draft/approval mode: require_approval → pending_approval → client approves in portal or via SMS", "SMS via Twilio: notify_phone config, draft notification + FYI on auto-send, OK/SKIP webhook", "AI Voice Training corpus: lead_conversations table, thread upload, Claude voice extraction pipeline", "from_email validation relaxed: emailAccount SMTP is accepted as alternative"] } } },
          ...
          { id: "l2-workflow", type: "subNode", position: { x: 0,   y: 310 }, data: { label: "workflow.ts",       type: "lib-file", hint: "buildSystemPrompt() — voice samples + rules" } },
          { id: "l2-tools",    type: "subNode", position: { x: 130, y: 310 }, data: { label: "tools.ts",          type: "lib-file", hint: "sendEmail() · extractSmtpConfig() · extractImapConfig()" } },
          { id: "l2-twilio",   type: "subNode", position: { x: 260, y: 310 }, data: { label: "Twilio SMS",        type: "external-service", hint: "lib/twilio.ts — sendSms + webhook validation" } },
          { id: "l2-claude",   type: "subNode", position: { x: 0,   y: 410 }, data: { label: "Claude API",        type: "external-service", hint: "Haiku (parse/upload) · Sonnet (reply/extract)" } },
          { id: "l2-runs-db",  type: "subNode", position: { x: 130, y: 410 }, data: { label: "automation_runs",   type: "db-table", hint: "queued | pending_approval | approved | success | error | discarded" } },
          { id: "l2-conv-db",  type: "subNode", position: { x: 260, y: 410 }, data: { label: "lead_conversations",type: "db-table", hint: "inbound + outbound · was_edited flag · voice training corpus" } },
        ],
        subEdges: [
          { id: "e-cr-re",  source: "l2-cron",     target: "l2-registry",  type: "smoothstep" },
          ...
          { id: "e-le-tw",  source: "l2-lead",      target: "l2-twilio",    type: "smoothstep", label: "notify" },
          ...
          { id: "e-le-cv",  source: "l2-lead",      target: "l2-conv-db",   type: "smoothstep", label: "corpus" },
        ...
        targetState: [
          "Lead Reply fully working on Casper's account with real inbound email",
          "Voice samples auto-linked from Knowledge Hub entries into automation config",
          "Social Poster and Review Collector production-tested",
        ],
        plan: [
          "Test Lead Reply end-to-end on Casper's inbox (real inbound email, watch Runs log)",
          "Auto-link Knowledge Hub voice entries → automations.config.voice_samples on save",
          "Social Poster: scaffold index.ts + tools.ts + obtain Facebook token",
          "Review Collector: production test",
        ],
        changelog: [
          "SMS approval via Twilio: notify_phone field, OK/SKIP webhook, FYI on auto-send",
          "AI Voice Training corpus: lead_conversations table, thread upload (Haiku), voice extraction (Sonnet)",
          "IMAP polling replaces Resend inbound webhook — check-leads cron, ImapFlow, returning contacts check",
          "Delayed queue: reply_delay_minutes → queued → Phase 1 processes on next tick",
          "Draft/approval mode fully wired: require_approval → pending_approval → approve in portal or via SMS",
          "n8n completely removed — replaced with TypeScript WAT pattern",
          "automation_runs + lead_conversations tables applied to prod",
        ],
      },
    },
  },
